...
if (isset($_GET['id'])) {
    $dados = $oficina->buscarPorId((int) $_GET['id']);
    if ($dados) {
        $acao = 'atualizar';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($acao === 'cadastrar') {
            $oficina->cadastrar($payload);
        } else {
            $oficina->atualizar((int) $_POST['id'], $payload);
        }
        ...
        $titulo = $acao === 'atualizar' ? 'Editar Oficina' : 'Nova Oficina';
    }
}
        ...
        <div class="card-body">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="text-end">
                    ...
                    <a href="oficinas.php" class="btn btn-secondary">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
