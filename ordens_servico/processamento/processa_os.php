...
try {
    ...
    /** Volta ao formulário preservando tipo e equipamento na criação (evita perder a query string e dar "Tipo de equipamento inválido"). */
    $url_form_os = function () use ($modo_edicao) {
        if ($modo_edicao) {
            return '../os.php?id=' . (int) ($_POST['id'] ?? 0);
        }
        $tipo = $_POST['tipo_equipamento'] ?? '';
        $eq = (int) ($_POST['equipamento_id'] ?? 0);
        if ($tipo !== '' && $eq > 0) {
            return '../os.php?tipo=' . rawurlencode($tipo) . '&id_equipamento=' . $eq;
        }

        return '../ordens_servico.php';
    };
    ...
    if (!$modo_edicao) {
        ...
        $campos_obrigatorios['status'] = 'Status da OS';
    }
    ...
    if (!empty($erros)) {
        ...
        header('Location: ' . $url_form_os());
        exit;
    }

    // Validação específica para cliente terceiro
    if ($_POST['tipo_proprietario'] === 'terceiro' && empty($_POST['cliente_id'])) {
        $erros[] = 'O campo Executor Terceiro é obrigatório quando o proprietário for terceiro.';
    }
