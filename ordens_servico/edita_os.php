...
if (!function_exists('preditix_label_equipamento_edita_os')) {
    function preditix_label_equipamento_edita_os($tipo, array $eq)
    {
        switch ($tipo) {
            case 'embarcacao':
                return $eq['nome'] ?? '';
...
try {
    ...
    $sql_patios = "SELECT id, nome FROM patios WHERE status = 'ativo' ORDER BY nome";
    $equipamentos['patio'] = $db->query($sql_patios);

    $sql_oficinas = "SELECT id, nome FROM oficinas WHERE status = 'ativo' ORDER BY nome";
    $equipamentos['oficina'] = $db->query($sql_oficinas);

    $sql_escritorios = "SELECT id, nome FROM escritorios WHERE status = 'ativo' ORDER BY nome";
    $equipamentos['escritorio'] = $db->query($sql_escritorios);
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="tipo_equipamento" name="tipo_equipamento" required>
                                        ...
                                        <option value="patio" <?php echo $os['tipo_equipamento'] === 'patio' ? 'selected' : ''; ?>>Pátio</option>
                                        <option value="oficina" <?php echo $os['tipo_equipamento'] === 'oficina' ? 'selected' : ''; ?>>Oficina</option>
                                        <option value="escritorio" <?php echo $os['tipo_equipamento'] === 'escritorio' ? 'selected' : ''; ?>>Escritório</option>
                                    </select>
                                </div>
                                    ...
                                    <select class="form-select" id="equipamento_id" name="equipamento_id" required>
                                        ...
                                        <?php foreach ($equipamentos[$os['tipo_equipamento']] as $equipamento): ?>
                                            <option value="<?php echo $equipamento['id']; ?>" 
                                                ...
                                                <?php echo htmlspecialchars(preditix_label_equipamento_edita_os($os['tipo_equipamento'], $equipamento)); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
        ...
        fetch(`processamento/busca_equipamentos.php?tipo=${tipo}`)
            ...
            .then(data => {
                ...
                data.equipamentos.forEach(equip => {
                    ...
                    option.textContent = equip.identificacao;
